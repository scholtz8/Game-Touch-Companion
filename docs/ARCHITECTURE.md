...
Se cancelan popups automáticos, descargas, permisos, esquemas externos, autenticación HTTP y selección de certificados. Los certificados inválidos nunca se aceptan. Se deshabilitan host objects, diálogos JavaScript predeterminados, menús contextuales, devtools, aceleradores de navegador, autocompletado y guardado de contraseñas. Los mensajes web están habilitados únicamente para un puente interno de teclado táctil: cada WebView recibe un token efímero y el host solo acepta mensajes de mostrar/ocultar teclado que incluyan ese token; no se aceptan comandos arbitrarios desde las páginas. Estas restricciones no prueban el foco de todos los sitios o controles nativos.
